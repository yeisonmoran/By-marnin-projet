...
if ($_POST) {
    ...
    if ($producto['stock'] >= $cantidad) {
        ...
        // 3. Restar stock en la tabla productos
        $nuevo_stock = $producto['stock'] - $cantidad;
        $conexion->query("UPDATE productos SET stock = $nuevo_stock WHERE id_producto = $id_producto");

        // 4. CREAR EL REGISTRO EN LA TABLA VENTAS
        // Asumiendo que la tabla ventas tiene id_cliente, fecha y estado según discutimos.
        // Si tu tabla ventas tiene también la columna 'cantidad' o 'id_producto', puedes agregarla al INSERT
        $sql_venta = "INSERT INTO ventas (id_cliente, fecha, estado) VALUES ('$id_cliente', '$fecha', '$estado')";
        
        if ($conexion->query($sql_venta)) {
            $mensaje = "<div class='alert alert-success'>Venta registrada y stock actualizado correctamente.</div>";
        } else {
            $mensaje = "<div class='alert alert-danger'>Stock actualizado, pero error al registrar venta: " . $conexion->error . "</div>";
        }
